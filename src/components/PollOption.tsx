...
export const PollOption: React.FC<PollOptionProps> = ({
  ...
  return (
    <div
      ...
      } ${hasPollEnded ? "pointer-events-none !border-none !py-3 !px-0" : ""}`}
    ...
    >
      ...
      <div className="w-full bg-neutral-200 rounded-full h-2 relative overflow-hidden">
        <div
          ...
          className={`absolute left-0 bg-gradient-to-r from-emerald-500 to-green-500 h-full rounded-full animate_progress transition-[width]`}
        ></div>
      </div>
    </div>
  );
